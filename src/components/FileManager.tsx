...
export const FileManager: React.FC<FileManagerProps> = ({
  ...
  return (
    <div className="w-full space-y-4 sm:space-y-6 overflow-x-hidden">
      ...
      <div className="bg-gradient-to-r from-[#0F1E42] via-[#1B357B] to-[#25469B] rounded-2xl sm:rounded-3xl p-5 sm:p-7 shadow-md relative overflow-hidden flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 lg:gap-8 text-white w-full max-w-full">
        {/* Lado Esquerdo (Títulos e Descrição) */}
        <div className="relative z-10 space-y-1 w-full lg:max-w-xl">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 text-blue-100 text-xs font-semibold uppercase tracking-wider mb-2">
          ...
          <p className="text-blue-100/90 text-xs sm:text-sm max-w-xl font-normal leading-relaxed mt-1">
      ...
      <div className="w-full max-w-full overflow-hidden p-3 sm:p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-xs flex flex-col gap-3">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 flex-1 max-w-2xl">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full h-11 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 pl-10 pr-9 text-sm focus:bg-white dark:focus:bg-slate-900 focus:border-blue-500 dark:focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 outline-hidden transition-all shadow-2xs placeholder:text-slate-400 dark:placeholder:text-slate-500 text-slate-900 dark:text-slate-100"
              />
              {searchQuery && (
                <button
                  type="button"
            ...
            {onOpenCompanyModal && (
              <button
                ...
                className="w-full sm:w-auto h-11 px-4 rounded-xl border border-slate-200 dark:border-slate-700 font-semibold text-xs flex items-center justify-center gap-2 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-750 text-slate-700 dark:text-slate-200 transition-all shadow-2xs shrink-0 cursor-pointer group"
          ...
          <div className="flex flex-row items-center gap-2 w-full lg:w-auto justify-between lg:justify-end shrink-0">
            {canCreateSubfolder && (
              <button
                ...
                className="flex-1 lg:flex-initial h-11 px-3 sm:px-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 font-semibold text-xs flex items-center justify-center gap-1.5 shadow-2xs text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-750 transition-all cursor-pointer group"
              >
            ...
            {isQuotaExceeded ? (
              ...
              <div className="flex-1 lg:flex-initial h-11 px-3 sm:px-4 text-xs text-slate-400 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl flex items-center justify-center gap-1.5" title="Apenas usuários com papel Editor ou Admin nesta pasta podem fazer upload.">
            ...
            <div className="flex items-center bg-slate-100 dark:bg-slate-950 p-1 rounded-xl h-11 gap-1 flex-shrink-0 border border-slate-200 dark:border-slate-800 shadow-2xs">
              <button
                ...
                className={`w-9 h-9 flex items-center justify-center rounded-lg transition-all cursor-pointer ${viewMode === 'grid' ? 'bg-white dark:bg-slate-800 text-[#1B357B] dark:text-[#E2C37A] shadow-xs' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
              ...
              <button
                ...
                className={`w-9 h-9 flex items-center justify-center rounded-lg transition-all cursor-pointer ${viewMode === 'list' ? 'bg-white dark:bg-slate-800 text-[#1B357B] dark:text-[#E2C37A] shadow-xs' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
        ...
        <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-slate-100 text-xs text-slate-600">
          {searchQuery && (
            <>
              <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-blue-50 text-blue-700 border border-blue-200 text-xs font-semibold shrink-0">
                ...
                <button
                  ...
                  className="p-0.5 hover:bg-blue-100 rounded-md text-blue-600 cursor-pointer"
          ...
          <span className="text-xs font-bold text-slate-800 mr-1 flex items-center space-x-1 shrink-0">
            <Filter className="w-3.5 h-3.5 flex-shrink-0 text-blue-600" />
            <span>Filtro de Período:</span>
          </span>
          <select
            value={selectedMonth}
            onChange={(e) => setSelectedMonth(e.target.value)}
            className="h-9 px-3 rounded-xl border border-slate-200 bg-white text-xs font-semibold text-slate-700 focus:border-blue-600 outline-hidden cursor-pointer shadow-2xs"
          >
          ...
          {(selectedMonth !== 'ALL' || selectedYear !== 'ALL') && (
            <>
              ...
              <button
                ...
                className="h-9 px-3 rounded-xl text-xs font-semibold text-slate-600 hover:text-slate-900 hover:bg-slate-100 transition-all cursor-pointer flex items-center"
              >
                Limpar Filtro
              </button>
            </>
      ...
      <nav className="flex items-center space-x-2 text-xs font-semibold text-slate-700 bg-white px-4 py-3 rounded-2xl border border-slate-200 overflow-x-auto shadow-2xs">
        <button
          onClick={() => setCurrentFolderId(null)}
          className={`flex items-center space-x-1.5 hover:text-blue-600 transition-colors cursor-pointer ${!currentFolderId ? 'font-bold text-slate-900' : 'text-slate-700'}`}
        >
        ...
        {breadcrumbs.map((crumb, idx) => (
          <React.Fragment key={crumb.id}>
            ...
            <button
              ...
              className={`hover:text-blue-600 transition-colors whitespace-nowrap cursor-pointer ${idx === breadcrumbs.length - 1 ? 'font-bold text-slate-900' : 'text-slate-600'}`}
            >
              {crumb.name}
            </button>
          </React.Fragment>
        ...
        {currentFolder && isFullAdmin && !searchQuery && (
          <button
            ...
            className="ml-auto inline-flex items-center space-x-1.5 px-3 py-1.5 rounded-xl text-xs font-bold bg-blue-50 text-blue-700 hover:bg-blue-100 border border-blue-200 transition-colors shrink-0 cursor-pointer shadow-2xs"
      ...
      {!searchQuery && visibleFolders.length > 0 && (
        <section className="space-y-3 w-full">
          <h3 className="text-xs font-bold uppercase tracking-wider text-slate-800">Pastas no Nível Atual</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 w-full">
            {visibleFolders.map(folder => (
              <div
                key={folder.id}
                onClick={() => setCurrentFolderId(folder.id)}
                className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 p-4 shadow-xs hover:border-[#1B357B]/40 dark:hover:border-[#E2C37A]/40 hover:shadow-md transition-all flex items-center justify-between group cursor-pointer min-w-0 overflow-hidden"
              >
                <div className="flex items-center space-x-3.5 min-w-0 flex-1">
                  <div className="w-11 h-11 rounded-xl bg-blue-50 dark:bg-slate-800 text-[#1B357B] dark:text-[#E2C37A] border border-blue-100 dark:border-slate-700 group-hover:bg-[#1B357B] group-hover:text-white transition-all flex items-center justify-center font-bold shrink-0">
                    <FolderIcon className="w-5 h-5" />
                  </div>
                  <div className="truncate flex-1 min-w-0">
                    <h4 className="text-sm font-bold text-slate-800 dark:text-slate-100 group-hover:text-[#1B357B] dark:group-hover:text-[#E2C37A] transition-colors truncate">
                      {folder.name}
                    </h4>
                    <div className="flex items-center gap-1.5 mt-0.5">
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-medium truncate">{folder.sector}</span>
                      {isFullAdmin && (
                        Array.isArray(folder.allowed_user_ids) && folder.allowed_user_ids.length > 0 ? (
                          ...
                          <span className="inline-flex items-center gap-0.5 px-1.5 py-0.2 rounded text-[10px] font-semibold bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700" title="Apenas Administradores e Diretoria têm acesso">
                ...
                <div className="flex items-center space-x-1 shrink-0 ml-2">
                  {isFullAdmin && (
                    <button
                      ...
                      className="text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors p-1.5 rounded-lg cursor-pointer"
                  ...
                  <ChevronRight className="w-4 h-4 text-slate-400 group-hover:text-[#1B357B] dark:group-hover:text-[#E2C37A] transition-colors" />
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2 text-xs text-slate-600 font-semibold">
            ...
            <select
              ...
              className="bg-white border border-slate-200 rounded-xl px-2.5 py-1.5 font-semibold text-slate-700 text-xs shadow-2xs focus:ring-1 focus:ring-blue-600 cursor-pointer outline-none"
            >
        ...
        {sortedFiles.length === 0 ? (
          <div className="bg-white dark:bg-slate-900 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-800 p-12 text-center flex flex-col items-center justify-center gap-3">
            <div className="relative">
              <div className="w-16 h-16 rounded-2xl bg-blue-50 dark:bg-slate-800 border border-blue-100 dark:border-slate-700 flex items-center justify-center">
                <FileText className="w-8 h-8 text-[#1B357B] dark:text-[#E2C37A]" />
              </div>
            </div>
            <div className="max-w-sm">
              <h4 className="text-base font-bold text-slate-800 dark:text-slate-200">Nenhum documento encontrado</h4>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 w-full">
            {sortedFiles.map(file => {
              ...
              return (
                <div
                  key={file.id}
                  className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-xs hover:border-[#1B357B]/40 dark:hover:border-[#E2C37A]/40 hover:shadow-md transition-all p-4 sm:p-5 flex flex-col justify-between group relative min-w-0 overflow-hidden"
                >
                  <div>
                    ...
                    <div className="flex items-start justify-between">
                      <div className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 ${
                        ...
                        isImage ? 'bg-blue-50 dark:bg-blue-950/60 text-blue-600 dark:text-blue-400 border border-blue-100 dark:border-blue-900/50' :
                        (isSpreadsheet || isXml) ? 'bg-emerald-50 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-900/50' :
                        isPfx ? 'bg-purple-50 dark:bg-purple-950/60 text-purple-600 dark:text-purple-300 border border-purple-100 dark:border-purple-900/50' :
                        isZip ? 'bg-amber-50 dark:bg-amber-950/60 text-amber-600 dark:text-amber-400 border border-amber-100 dark:border-amber-900/50' :
                      ...
                      {isPfx ? (
                        <span className="bg-purple-50 dark:bg-purple-950/60 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-800/80 text-[10px] font-semibold px-2.5 py-0.5 rounded-full flex items-center gap-1">
                          <KeyRound className="w-3 h-3 text-purple-500 dark:text-purple-400" />
                          <span>Certificado</span>
                        </span>
                      ) : file.compression_ratio > 0 ? (
                        <span className="bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800/80 text-[10px] font-semibold px-2.5 py-0.5 rounded-full flex items-center gap-1">
                    ...
                    <div className="min-w-0">
                      <h4 
                        ...
                        className="text-sm font-bold text-slate-800 dark:text-slate-100 break-words line-clamp-2 group-hover:text-[#1B357B] dark:group-hover:text-[#E2C37A] transition-colors mt-3 cursor-pointer" 
                        title={file.name}
                      >
                        {file.name}
                      </h4>
                      <p className="text-xs text-slate-500 dark:text-slate-400 font-medium mb-3 flex items-center gap-1 min-w-0 truncate">
                        ...
                        {file.tags && file.tags.find(t => t.startsWith('Ref: ')) && (
                          <span className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-[10px] px-1.5 py-0.5 rounded font-medium ml-1 shrink-0">
                    ...
                    {/* Size Pill */}
                    <div className="text-xs font-semibold text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 px-2.5 py-1 rounded-lg w-fit mb-3">
                    ...
                    {file.tags && file.tags.filter(t => !t.startsWith('Ref: ')).length > 0 && (
                      <div className="flex flex-wrap gap-1.5">
                        {file.tags.filter(t => !t.startsWith('Ref: ')).slice(0, 3).map(tag => (
                          <span key={tag} className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 text-[11px] font-medium px-2 py-0.5 rounded-md">
                            #{tag}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="pt-3 mt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-1">
                      <button
                        type="button"
                        onClick={() => onOpenFileViewer(file)}
                        className="p-2 rounded-lg text-slate-400 hover:text-[#1B357B] dark:hover:text-[#E2C37A] hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer"
                    ...
                    <tr key={file.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors">
                      <td className="p-3.5 font-medium text-slate-900 dark:text-white">
                        <div className="flex items-center space-x-2.5">
                          ...
                          <span 
                            onClick={() => onOpenFileViewer(file)}
                            className="font-bold hover:text-[#1B357B] dark:hover:text-[#E2C37A] cursor-pointer truncate max-w-xs"
                          >
                            {file.name}
                          </span>
                          {isPfx && (
                            <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-purple-100 dark:bg-purple-950 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-800">
                              PFX
                            </span>
                          )}
                          {isXml && (
                            <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-amber-100 dark:bg-amber-950 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-800">
                              XML
                            </span>
                          )}
                        </div>
                      </td>
                      <td className="p-3.5 text-slate-600 dark:text-slate-300">{file.sector}</td>

                      <td className="p-3.5 font-mono text-slate-400 line-through">{formatBytes(file.original_size)}</td>
                      <td className="p-3.5 font-mono font-bold text-slate-700 dark:text-slate-200">{formatBytes(file.optimized_size)}</td>
                      <td className="p-3.5 font-bold text-emerald-600 dark:text-emerald-400">
                        {file.compression_ratio > 0 ? `-${file.compression_ratio}%` : '100% Íntegro'}
                      </td>
                      <td className="p-3.5 text-slate-500 dark:text-slate-400">{new Date(file.created_at).toLocaleDateString('pt-BR')}</td>
          ...
          <div className="bg-white w-full max-w-lg mx-auto rounded-2xl shadow-2xl border border-slate-200 flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in-95 duration-150 text-slate-900">
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3 py-2 text-xs bg-slate-50 border border-slate-200 rounded-xl text-slate-900 placeholder:text-slate-400 focus:bg-white focus:border-blue-600 outline-none transition-all font-medium"
                  />
                </div>
                ...
                <div className="flex flex-wrap gap-2 shrink-0">
                  <button
                    ...
                    className="flex-1 sm:flex-none px-3 py-2 text-xs font-bold text-blue-700 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-xl transition-colors cursor-pointer text-center whitespace-nowrap"
                  >
                    Marcar Filtrados
                  </button>
                  <button
                    ...
                    className="flex-1 sm:flex-none px-3 py-2 text-xs font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 border border-slate-200 rounded-xl transition-colors cursor-pointer text-center whitespace-nowrap"
                  >
                    Desmarcar Todos
                  </button>
                </div>
              ...
              <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar py-1 w-full text-xs">
                {(['ALL', 'Fiscal', 'Departamento Pessoal', 'Contábil', 'Financeiro', 'Geral'] as const).map(sec => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-xl text-[11px] font-bold transition-all cursor-pointer shrink-0 whitespace-nowrap ${
                  ...
                  return (
                    <div
                      ...
                      className={`py-2.5 px-3 rounded-xl flex items-center justify-between gap-2 cursor-pointer transition-colors ${
                        isAllowed
                          ? 'bg-emerald-50/70 hover:bg-emerald-50 border border-emerald-200/80'
                    ...
                    >
                      <div className="flex items-center gap-2.5 min-w-0 flex-1">
                        ...
                        />
                        <div className="w-8 h-8 rounded-full bg-slate-100 text-slate-700 font-bold text-xs flex items-center justify-center shrink-0 border border-slate-200 overflow-hidden">
                          {user.avatar_url ? (
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="text-xs font-bold text-slate-900 truncate max-w-[130px] sm:max-w-none">{user.full_name}</p>
                          <p className="text-[11px] text-slate-500 truncate max-w-[130px] sm:max-w-none">{user.email}</p>
                        </div>
                      </div>

                      <div className="flex items-center gap-1.5 shrink-0">
                        <span className="text-[10px] font-semibold text-slate-600 bg-slate-100 px-2 py-0.5 rounded border border-slate-200 whitespace-nowrap">
                          {user.sector}
                        </span>
                        {isAllowed ? (
                          <span className="inline-flex items-center gap-1 text-[10px] sm:text-[11px] font-bold text-emerald-800 bg-emerald-100 px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-lg border border-emerald-300 whitespace-nowrap">
                            <Check className="w-3 h-3 text-emerald-600 shrink-0" />
                            <span>Autorizado</span>
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1 text-[10px] sm:text-[11px] font-medium text-slate-600 bg-slate-100 px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-lg border border-slate-200 whitespace-nowrap">
            ...
            <div className="px-4 sm:px-6 py-3.5 border-t border-slate-200 bg-slate-50 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shrink-0">
              <div className="text-xs text-slate-600 flex items-center justify-between sm:justify-start">
                <div>
                  <span className="font-bold text-slate-900">{selectedAllowedUserIds.length}</span> de <span className="font-bold text-slate-900">{commonUsers.length}</span> autorizados
                </div>
                {permsSuccessMessage && (
                  <span className="ml-2 font-bold text-emerald-600 animate-in fade-in">{permsSuccessMessage}</span>
                )}
              </div>
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto">
                <button
                  ...
                  className="flex-1 sm:flex-none px-4 py-2.5 min-h-[40px] bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200 rounded-xl text-xs font-semibold cursor-pointer transition-colors text-center"
                >
                  Cancelar
                </button>
                <button
